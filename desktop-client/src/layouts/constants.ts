...
export const PYPI_URL = "https://pypi.org/pypi/sealclaw/json";

export const GITHUB_URL = "https://github.com/agentscope-ai/SealClaw" as const;
...
export const getDocsUrl = (lang: string): string =>
  `https://sealclaw.io/docs/intro?lang=${getWebsiteLang(lang)}`;

export const getFaqUrl = (lang: string): string =>
  `https://sealclaw.io/docs/faq?lang=${getWebsiteLang(lang)}`;

export const getReleaseNotesUrl = (lang: string): string =>
  `https://sealclaw.io/release-notes?lang=${getWebsiteLang(lang)}`;
...
docker run -p 127.0.0.1:8088:8088 -v sealclaw-data:/app/working agentscope/sealclaw:latest
\`\`\`

升级后重启服务 sealclaw app。`,

  ru: `### Как обновить SealClaw

Чтобы обновить SealClaw, выберите способ в зависимости от типа установки:
...
После обновления перезапустите сервис с помощью \`sealclaw app\`.`,

  en: `### How to update SealClaw
