...
REM SealClaw Installer for Windows (cmd.exe / batch)
REM Usage: install.bat [-Version X.Y.Z] [-FromSource] [-SourceDir DIR]
REM                         [-Extras "llamacpp,mlx"] [-UvPath PATH] [-Help]
REM
REM Installs SealClaw into %USERPROFILE%\.sealclaw with a uv-managed Python environment.
...
REM ── Defaults ──────────────────────────────────────────────────────────────────
if defined SEALCLAW_HOME (
    set "SEALCLAW_HOME=%SEALCLAW_HOME%"
) else (
    set "SEALCLAW_HOME=%USERPROFILE%\.sealclaw"
)
set "SEALCLAW_VENV=%SEALCLAW_HOME%\venv"
set "SEALCLAW_BIN=%SEALCLAW_HOME%\bin"
set "PYTHON_VERSION=3.12"
set "SEALCLAW_REPO=https://github.com/agentscope-ai/SealClaw.git"
...
REM ──── Help ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
:show_help
echo SealClaw Installer for Windows
echo.
...
echo.
echo Environment:
echo   SEALCLAW_HOME            Installation directory (default: %%USERPROFILE%%\.sealclaw)
exit /b 0
...
:write_warn
echo [sealclaw] WARNING: %~1
exit /b 0

:write_err
echo [sealclaw] ERROR: %~1
exit /b 0
...
echo [sealclaw] Downloading uv ^(!_DL_ARCH!^) from GitHub Releases...
...
if not errorlevel 1 (
    ...
    echo [sealclaw] curl failed, retrying with PowerShell...
    del "!_DL_ZIP!" >nul 2>&1
)
...
if errorlevel 1 (
    echo [sealclaw] ERROR: GitHub download also failed.
    echo [sealclaw] Download uv manually from: https://github.com/astral-sh/uv/releases/latest
    del "!_DL_ZIP!" >nul 2>&1
    exit /b 1
)
...
echo [sealclaw] Extracting uv...
powershell -NoProfile -Command "Expand-Archive -Force -Path '!_DL_ZIP!' -DestinationPath '!_DL_DEST!'"
set "_DL_ERR=%errorlevel%"
del "!_DL_ZIP!" >nul 2>&1
if %_DL_ERR% neq 0 (
    echo [sealclaw] ERROR: Extraction failed.
    exit /b 1
)
if not exist "!_DL_DEST!\uv.exe" (
    echo [sealclaw] ERROR: uv.exe not found after extraction.
    exit /b 1
)
set "PATH=!_DL_DEST!;!PATH!"
echo [sealclaw] uv installed: !_DL_DEST!\uv.exe
exit /b 0
...
if defined ARG_UV_PATH (
    if not exist "%ARG_UV_PATH%" (
        echo [sealclaw] ERROR: Specified uv not found: %ARG_UV_PATH%
        exit /b 1
    )
    for %%I in ("%ARG_UV_PATH%") do set "PATH=%%~dpI;!PATH!"
    echo [sealclaw] uv found: %ARG_UV_PATH%
    goto :ensure_uv_done
)
...
for %%c in ("%USERPROFILE%\.local\bin\uv.exe" "%USERPROFILE%\.cargo\bin\uv.exe" "%LOCALAPPDATA%\uv\uv.exe") do (
    if exist %%c (
        ...
        echo [sealclaw] uv found: %%~c
        goto :ensure_uv_done
    )
)

REM 3. Try astral.sh (standard installer, fast outside China)
echo [sealclaw] Installing uv via astral.sh...
...
echo [sealclaw] astral.sh unreachable, falling back to GitHub Releases...
call :download_uv_github
if errorlevel 1 (
    echo [sealclaw] ERROR: Failed to install uv automatically.
    echo [sealclaw] Please install uv manually: https://docs.astral.sh/uv/
    exit /b 1
)
goto :ensure_uv_done
...
where uv >nul 2>&1
if errorlevel 1 (
    echo [sealclaw] ERROR: Failed to install uv. Please install it manually: https://docs.astral.sh/uv/
    exit /b 1
)
echo [sealclaw] uv installed via astral.sh
...
set "_CONSOLE_DEST=%_REPO_DIR%\src\sealclaw\console"
...
if exist "%_CONSOLE_SRC%\index.html" (
    echo [sealclaw] Copying console frontend assets...
...
if not exist "%_REPO_DIR%\console\package.json" (
    echo [sealclaw] WARNING: Console source not found - the web UI won't be available.
    exit /b 0
)

where npm >nul 2>&1
if errorlevel 1 (
    echo [sealclaw] WARNING: npm not found - skipping console frontend build.
    echo [sealclaw] WARNING: Install Node.js from https://nodejs.org/ then re-run this installer,
    echo [sealclaw] WARNING: or run 'cd console ^&^& npm ci ^&^& npm run build' manually.
    exit /b 0
)

echo [sealclaw] Building console frontend (npm ci ^&^& npm run build)...
pushd "%_REPO_DIR%\console"
npm ci
if errorlevel 1 (
    popd
    echo [sealclaw] WARNING: npm ci failed - the web UI won't be available.
    exit /b 0
)
npm run build
if errorlevel 1 (
    popd
    echo [sealclaw] WARNING: npm run build failed - the web UI won't be available.
    exit /b 0
)
popd
...
if exist "%_CONSOLE_SRC%\index.html" (
    ...
    set "CONSOLE_COPIED=1"
    set "CONSOLE_AVAILABLE=1"
    echo [sealclaw] Console frontend built successfully
    exit /b 0
)

echo [sealclaw] WARNING: Console build completed but index.html not found - the web UI won't be available.
exit /b 0
...
REM %~1 = RepoDir
if "%CONSOLE_COPIED%"=="1" (
    set "_CLEANUP_DEST=%~1\src\sealclaw\console"
...
REM ══════════════════════════════ MAIN ═════════════════════════════════════════
:main
echo [sealclaw] Installing SealClaw into %SEALCLAW_HOME%
...
if exist "%SEALCLAW_VENV%" (
    echo [sealclaw] Existing environment found, upgrading...
) else (
    echo [sealclaw] Creating Python %PYTHON_VERSION% environment...
)

uv venv "%SEALCLAW_VENV%" --python %PYTHON_VERSION% --quiet --clear
if errorlevel 1 (
    echo [sealclaw] ERROR: Failed to create virtual environment
    exit /b 1
)

set "VENV_PYTHON=%SEALCLAW_VENV%\Scripts\python.exe"
...
echo [sealclaw] Python environment ready (%PY_VERSION%)

REM ──── Step 3: Install SealClaw ──────────────────────────────────────────────────────────────────────────────────────────────────────────
set "EXTRAS_SUFFIX="
if defined ARG_EXTRAS set "EXTRAS_SUFFIX=[%ARG_EXTRAS%]"

set "VENV_COPAW=%SEALCLAW_VENV%\Scripts\sealclaw.exe"
...
goto :install_from_github_sealclaw

:install_from_local
for %%I in ("%ARG_SOURCE_DIR%") do set "ARG_SOURCE_DIR=%%~fI"
echo [sealclaw] Installing SealClaw from local source: %ARG_SOURCE_DIR%
call :prepare_console "%ARG_SOURCE_DIR%"
echo [sealclaw] Installing package from source...
...
call :cleanup_console "%ARG_SOURCE_DIR%"
if %_INST_ERR% neq 0 (
    echo [sealclaw] ERROR: Installation from source failed
    exit /b 1
)
goto :install_verify

:install_from_github_sealclaw
where git >nul 2>&1
if errorlevel 1 (
    echo [sealclaw] ERROR: git is required for -FromSource without a local directory.
    echo [sealclaw]        Please install Git from https://git-scm.com/ or pass a local path:
    echo [sealclaw]        install-w-uv.bat -FromSource -SourceDir C:\path\to\SealClaw
    exit /b 1
)
echo [sealclaw] Installing SealClaw from source (GitHub)...
set "CLONE_DIR=%TEMP%\sealclaw-install-%RANDOM%"
git clone --depth 1 %SEALCLAW_REPO% "%CLONE_DIR%"
if errorlevel 1 (
    if exist "%CLONE_DIR%" rd /s /q "%CLONE_DIR%"
    echo [sealclaw] ERROR: Failed to clone repository
    exit /b 1
)
...
if defined ARG_VERSION (
    ...
    set "_PACKAGE=sealclaw%ARG_VERSION%"
)
rem === End Version Validation ===

echo [sealclaw] Installing %_PACKAGE%%EXTRAS_SUFFIX% from PyPI...
...
uv pip install "%_PACKAGE%%EXTRAS_SUFFIX%" --python "%VENV_PYTHON%" --prerelease=allow --quiet --refresh-package sealclaw
if errorlevel 1 (
    echo [sealclaw] ERROR: Installation failed
    exit /b 1
)
...
if not exist "%VENV_COPAW%" (
    echo [sealclaw] ERROR: Installation failed: sealclaw CLI not found in venv
    exit /b 1
)
echo [sealclaw] SealClaw installed successfully

REM Check console availability (for PyPI installs, probe the installed package)
if "%CONSOLE_AVAILABLE%"=="0" (
    "%VENV_PYTHON%" -c "import importlib.resources, sealclaw; p=importlib.resources.files('sealclaw')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" > "%TEMP%\_sealclaw_console_check.tmp" 2>&1
    set /p CONSOLE_CHECK=<"%TEMP%\_sealclaw_console_check.tmp"
    del "%TEMP%\_sealclaw_console_check.tmp" >nul 2>&1
    if "!CONSOLE_CHECK!"=="yes" set "CONSOLE_AVAILABLE=1"
)

REM ──── Step 4: Create wrapper scripts ────────────────────────────────────────────────────────────────────────────────────────
if not exist "%SEALCLAW_BIN%" mkdir "%SEALCLAW_BIN%"

REM PowerShell wrapper
set "WRAPPER_PS1=%SEALCLAW_BIN%\sealclaw.ps1"
echo # SealClaw CLI wrapper -- delegates to the uv-managed environment. > "%WRAPPER_PS1%"
echo $ErrorActionPreference = "Stop" >> "%WRAPPER_PS1%"
echo. >> "%WRAPPER_PS1%"
echo $CopawHome = if ($env:SEALCLAW_HOME) { $env:SEALCLAW_HOME } else { Join-Path $HOME ".sealclaw" } >> "%WRAPPER_PS1%"
echo $RealBin = Join-Path $CopawHome "venv\Scripts\sealclaw.exe" >> "%WRAPPER_PS1%"
echo. >> "%WRAPPER_PS1%"
echo if (-not (Test-Path $RealBin)) { >> "%WRAPPER_PS1%"
echo     Write-Error "SealClaw environment not found at $CopawHome\venv" >> "%WRAPPER_PS1%"
echo     Write-Error "Please reinstall: irm ^<install-url^> ^| iex" >> "%WRAPPER_PS1%"
echo     exit 1 >> "%WRAPPER_PS1%"
echo } >> "%WRAPPER_PS1%"
echo. >> "%WRAPPER_PS1%"
echo ^& $RealBin @args >> "%WRAPPER_PS1%"
echo [sealclaw] Wrapper created at %WRAPPER_PS1%

REM CMD wrapper
set "WRAPPER_CMD=%SEALCLAW_BIN%\sealclaw.cmd"
echo @echo off > "%WRAPPER_CMD%"
echo REM SealClaw CLI wrapper -- delegates to the uv-managed environment. >> "%WRAPPER_CMD%"
echo set "SEALCLAW_HOME=%%SEALCLAW_HOME%%" >> "%WRAPPER_CMD%"
echo if "%%SEALCLAW_HOME%%"=="" set "SEALCLAW_HOME=%%USERPROFILE%%\.sealclaw" >> "%WRAPPER_CMD%"
echo set "REAL_BIN=%%SEALCLAW_HOME%%\venv\Scripts\sealclaw.exe" >> "%WRAPPER_CMD%"
echo if not exist "%%REAL_BIN%%" ( >> "%WRAPPER_CMD%"
echo     echo Error: SealClaw environment not found at %%SEALCLAW_HOME%%\venv ^>^&2 >> "%WRAPPER_CMD%"
echo     echo Please reinstall ^>^&2 >> "%WRAPPER_CMD%"
echo     exit /b 1 >> "%WRAPPER_CMD%"
echo ) >> "%WRAPPER_CMD%"
echo "%%REAL_BIN%%" %%* >> "%WRAPPER_CMD%"
echo [sealclaw] CMD wrapper created at %WRAPPER_CMD%
...
set "check_str=;%SEALCLAW_BIN%;"
if /i "%path_check%" neq "%path_check:%check_str%=%" (
    echo [sealclaw] %SEALCLAW_BIN% already in PATH
) else (
    :: === 修复1：安全传递参数（解决命令注入） ===
    if defined CURRENT_USER_PATH (
        powershell -NoProfile -Command "$p = $args[0]; $v = $args[1]; [Environment]::SetEnvironmentVariable('Path', $p + ';' + $v, 'User')" "%SEALCLAW_BIN%" "!CURRENT_USER_PATH!"
    ) else (
        powershell -NoProfile -Command "$p = $args[0]; [Environment]::SetEnvironmentVariable('Path', $p, 'User')" "%SEALCLAW_BIN%"
    )

    :: === 修复2：添加关键错误检查（解决失败不报错） ===
    if errorlevel 1 (
        echo [error] Failed to update PATH. SEALCLAW_BIN: "%SEALCLAW_BIN%"
        echo [error] Please verify the path is valid.
        exit /b 1
    )

    :: === 修复3：安全更新当前进程PATH ===
    set "PATH=%SEALCLAW_BIN%;!PATH!"
    echo [sealclaw] Added %SEALCLAW_BIN% to PATH
)

REM ──── Done ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
echo.
echo SealClaw installed successfully!
echo.
echo   Install location:  %SEALCLAW_HOME%
...
echo.
echo   sealclaw init       # first-time setup
echo   sealclaw app        # start SealClaw
echo.
echo To upgrade later, re-run this installer.
echo To uninstall, run: sealclaw uninstall
