...
- Builds the console frontend (`console/`), copies `console/dist` to `src/sealclaw/console/dist`, then builds the wheel. Output: `dist/*.whl`.
...
- Default tag: `sealclaw:latest`. Uses `deploy/Dockerfile` (multi-stage: builds console then Python app).
- Example: `bash scripts/docker_build.sh myreg/sealclaw:v1 --no-cache`.
