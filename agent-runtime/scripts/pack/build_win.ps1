...
# 清理过程不应阻断构建：conda 环境可能包含 junction/symlink，
# 删除目录后遍历残留路径可能抛出终止性错误。
...
try {
    ...
    if (Test-Path $condaMeta) {
        Remove-Item -Recurse -Force $condaMeta -ErrorAction SilentlyContinue
        Write-Host "[cleanup] Removed conda-meta/"
    }

    # 2. 删除各 Python 包内的测试目录
    $testDirs = Get-ChildItem -Path $EnvRoot -Recurse -Directory -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -in @("tests", "test", "_test", "_tests") -and $_.FullName -notmatch '\\Scripts\\' }
    $testDirCount = 0
    ...
    Write-Host "[cleanup] Removed $testDirCount test directories"

    # 3. 删除 .dist-info 中的 RECORD 和 INSTALLER（大量小文件，安装后无用）
    Get-ChildItem -Path $EnvRoot -Recurse -File -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -in @("RECORD", "INSTALLER") } |
    ...
    Write-Host "[cleanup] WARN: cleanup had errors (non-fatal): $_" -ForegroundColor Yellow
} finally {
