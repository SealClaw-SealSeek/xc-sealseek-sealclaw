# SealClaw Installer for Windows (self-contained: includes uv download via GitHub)
# Usage: irm <url>/install.ps1 | iex
#    or: .\install.ps1 [-Version X.Y.Z] [-FromSource] [-SourceDir DIR]
#                            [-Extras "llamacpp,mlx"] [-UvPath PATH]
#
# Installs SealClaw into ~/.sealclaw with a uv-managed Python environment.
...
# ── Defaults ──────────────────────────────────────────────────────────────────
$SealclawHome     = if ($env:SEALCLAW_HOME) { $env:SEALCLAW_HOME } else { Join-Path $HOME ".sealclaw" }
$CopawVenv     = Join-Path $SealclawHome "venv"
$CopawBin      = Join-Path $SealclawHome "bin"
$PythonVersion = "3.12"
$CopawRepo     = "https://github.com/agentscope-ai/SealClaw.git"

# ── Colors ────────────────────────────────────────────────────────────────────
function Write-Info { param([string]$Message) Write-Host "[sealclaw] " -ForegroundColor Green  -NoNewline; Write-Host $Message }
function Write-Warn { param([string]$Message) Write-Host "[sealclaw] " -ForegroundColor Yellow -NoNewline; Write-Host $Message }
function Write-Err  { param([string]$Message) Write-Host "[sealclaw] " -ForegroundColor Red    -NoNewline; Write-Host $Message }
function Stop-WithError { param([string]$Message) Write-Err $Message; exit 1 }

# ── Help ──────────────────────────────────────────────────────────────────────
if ($Help) {
    @"
SealClaw Installer for Windows
...
Environment:
  SEALCLAW_HOME            Installation directory (default: ~/.sealclaw)
"@
    exit 0
}

Write-Host "[sealclaw] " -ForegroundColor Green -NoNewline
Write-Host "Installing SealClaw into " -NoNewline
Write-Host "$SealclawHome" -ForegroundColor White
...
function Prepare-Console {
    ...
    $consoleDest = Join-Path $RepoDir "src\sealclaw\console"
...
$VenvSealclaw = Join-Path $CopawVenv "Scripts\sealclaw.exe"

if ($FromSource) {
    if ($SourceDir) {
        $SourceDir = (Resolve-Path $SourceDir).Path
        Write-Info "Installing SealClaw from local source: $SourceDir"
        ...
        if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
            Stop-WithError "git is required for -FromSource without a local directory. Please install Git from https://git-scm.com/ or pass a local path: .\install.ps1 -FromSource -SourceDir C:\path\to\SealClaw"
        }
        Write-Info "Installing SealClaw from source (GitHub)..."
        $cloneDir = Join-Path $env:TEMP "sealclaw-install-$(Get-Random)"
        try {
    ...
    $package = "sealclaw"
    if ($Version) { $package = "sealclaw==$Version" }

    Write-Info "Installing ${package}${ExtrasSuffix} from PyPI..."
    uv pip install "${package}${ExtrasSuffix}" --python $VenvPython --prerelease=allow --quiet --refresh-package sealclaw
    if ($LASTEXITCODE -ne 0) { Stop-WithError "Installation failed" }
}

# Verify the CLI entry point exists
if (-not (Test-Path $VenvSealclaw)) { Stop-WithError "Installation failed: sealclaw CLI not found in venv" }

Write-Info "SealClaw installed successfully"

# Check console availability (for PyPI installs, check the installed package)
if (-not $script:ConsoleAvailable) {
    $consoleCheck = & $VenvPython -c "import importlib.resources, sealclaw; p=importlib.resources.files('sealclaw')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>&1
...
$wrapperPath = Join-Path $CopawBin "sealclaw.ps1"
$wrapperContent = @'
# SealClaw CLI wrapper — delegates to the uv-managed environment.
$ErrorActionPreference = "Stop"

$SealclawHome = if ($env:SEALCLAW_HOME) { $env:SEALCLAW_HOME } else { Join-Path $HOME ".sealclaw" }
$RealBin   = Join-Path $SealclawHome "venv\Scripts\sealclaw.exe"

if (-not (Test-Path $RealBin)) {
    Write-Error "SealClaw environment not found at $SealclawHome\venv"
...
$cmdWrapperPath = Join-Path $CopawBin "sealclaw.cmd"
$cmdWrapperContent = @"
@echo off
REM SealClaw CLI wrapper — delegates to the uv-managed environment.
set "SEALCLAW_HOME=%SEALCLAW_HOME%"
if "%SEALCLAW_HOME%"=="" set "SEALCLAW_HOME=%USERPROFILE%\.sealclaw"
set "REAL_BIN=%SEALCLAW_HOME%\venv\Scripts\sealclaw.exe"
if not exist "%REAL_BIN%" (
    echo Error: SealClaw environment not found at %SEALCLAW_HOME%\venv >&2
...
if (-not $isAlreadyAdded) {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "ACTION REQUIRED: You must manually add the path to use SealClaw."
...
# ── Done ──────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "SealClaw installed successfully!" -ForegroundColor Green
Write-Host ""

Write-Host "  Install location:  " -NoNewline; Write-Host "$SealclawHome" -ForegroundColor White
...
Write-Host ""
Write-Host "  sealclaw init" -ForegroundColor White -NoNewline; Write-Host "       # first-time setup"
Write-Host "  sealclaw app"  -ForegroundColor White -NoNewline; Write-Host "        # start SealClaw"
Write-Host ""
Write-Host "To upgrade later, re-run this installer."
Write-Host "To uninstall, run: " -NoNewline
Write-Host "sealclaw uninstall" -ForegroundColor White
