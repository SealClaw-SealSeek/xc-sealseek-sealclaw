...
[[Documentation](https://sealclaw.io/)] [[中文 README](README_zh.md)] [[日本語](README_ja.md)]
...
[2026-03-12] We released v0.0.7! See the [v0.0.7 Release Notes](https://agentscope-ai.github.io/SealClaw/release-notes) for the full changelog.
...
[2026-03-09] We released v0.0.6! See the [v0.0.6 Release Notes](https://agentscope-ai.github.io/SealClaw/release-notes) for the full changelog.

[2026-03-06] We released v0.0.5! See the [v0.0.5 Release Notes](https://agentscope-ai.github.io/SealClaw/release-notes) for the full changelog.

[2026-03-02] We released v0.0.4! See the [v0.0.4 Release Notes](https://agentscope-ai.github.io/SealClaw/release-notes) for the full changelog.
...
> **Recommended reading:**
>
> - **I want to run SealClaw in 3 commands**: [Quick Start](#quick-start) → open Console in browser.
> - **I want to chat in DingTalk / Feishu / QQ**: Configure [channels](https://sealclaw.io/docs/channels) in the Console.
> - **I don’t want to install Python**: [Script install](#script-install) handles Python automatically, or use [ModelScope one-click](https://modelscope.cn/studios/fork?target=AgentScope/SealClaw) for cloud deployment.
...
Then open **http://127.0.0.1:8088/** in your browser for the Console (chat with SealClaw, configure the agent). To talk in DingTalk, Feishu, QQ, etc., add a channel in the [docs](https://sealclaw.io/docs/channels).
...
>      - SealClaw path: Typically located at `%USERPROFILE%\.sealclaw\bin`.
...
>   - **Configure `uv` environment variables**: Add the `uv` directory and `%USERPROFILE%\.sealclaw\bin` to your system's `Path` variable.
...
>   - **Configure the `SealClaw` environment variable**: Add `%USERPROFILE%\.sealclaw\bin` to your system's `Path` variable.
...
Download the desktop app from [GitHub Releases](https://github.com/agentscope-ai/SealClaw/releases):
...
- **Right-click to open (recommended)**
  Right-click (or Control+click) the SealClaw app → **Open** → in the dialog click **Open** again. This tells Gatekeeper you trust the app; after that you can double-click to launch as usual.

- **Allow in System Settings**
  If it is still blocked, go to **System Settings → Privacy & Security**, scroll to the message like *"SealClaw was blocked because it is from an unidentified developer"*, and click **Open Anyway** or **Allow**.
...
For detailed usage instructions, troubleshooting, and common issues, see the [Desktop Application Guide](https://sealclaw.io/docs/desktop).
...
**No local install?** [ModelScope Studio](https://modelscope.cn/studios/fork?target=AgentScope/SealClaw) one-click cloud setup. Set your Studio to **non-public** so others cannot control your SealClaw.

### Deploy on Alibaba Cloud ECS

To run SealClaw on Alibaba Cloud (ECS), use the one-click deployment: open the [SealClaw on Alibaba Cloud (ECS) deployment link](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884) and follow the prompts. For step-by-step instructions, see [Alibaba Cloud Developer: Deploy your AI assistant in 3 minutes](https://developer.aliyun.com/article/1713682).

---

## API Key

If you use a **cloud LLM** (e.g. DashScope, ModelScope), you must configure an API key before chatting. SealClaw will not work until a valid key is set. See the [official docs](https://sealclaw.io/docs/models#configure-cloud-providers) for details.

**How to configure:**

1. **Console (recommended)** — After running `sealclaw app`, open **http://127.0.0.1:8088/** → **Settings** → **Models**. Choose a provider, enter the **API Key**, and enable that provider and model.
...
Tools that need extra keys (e.g. `TAVILY_API_KEY` for web search) can be set in Console **Settings → Environment variables**, or see [Config](https://sealclaw.io/docs/config) for details.

> **Using local models only?** If you use [Local Models](#local-models) (llama.cpp or MLX), you do **not** need any API key.

## Local Models

SealClaw can run LLMs entirely on your machine — no API keys or cloud services required. See the [official docs](https://sealclaw.io/docs/models#local-providers-llamacpp--mlx) for details.
...
| Topic                                                                 | Description                                      |
| --------------------------------------------------------------------- | ------------------------------------------------ |
| [Introduction](https://sealclaw.io/docs/intro)                | What SealClaw is and how to use it                  |
| [Quick start](https://sealclaw.io/docs/quickstart)            | Install and run (local or ModelScope Studio)    |
| [Console](https://sealclaw.io/docs/console)                   | Web UI: chat and agent configuration            |
| [Models](https://sealclaw.io/docs/models)                     | Configure cloud, local, and custom providers    |
| [Channels](https://sealclaw.io/docs/channels)                  | DingTalk, Feishu, QQ, Discord, iMessage, and more |
| [Skills](https://sealclaw.io/docs/skills)                      | Extend and customize capabilities               |
| [MCP](https://sealclaw.io/docs/mcp)                            | Manage MCP clients                               |
| [Memory](https://sealclaw.io/docs/memory)                     | Long-term memory                     |
| [Context](https://sealclaw.io/docs/context)                   | Context management mechanism                     |
| [Magic commands](https://sealclaw.io/docs/commands)           | Control conversation state without waiting for the AI |
| [Heartbeat](https://sealclaw.io/docs/heartbeat)                | Scheduled check-in and digest                    |
| [Config & working dir](https://sealclaw.io/docs/config) | Working directory and config file                |
| [CLI](https://sealclaw.io/docs/cli)                            | Init, cron jobs, skills, clean                   |
| [FAQ](https://sealclaw.io/docs/faq)                           | Common questions and troubleshooting             |
...
For common questions, troubleshooting tips, and known issues, please visit the **[FAQ page](https://sealclaw.io/docs/faq)**.
...
We are building SealClaw in the open and welcome contributions of all kinds! Check the [Roadmap](#roadmap) above (especially items marked **Seeking Contributors**) to find areas that interest you, and read [CONTRIBUTING](https://github.com/agentscope-ai/SealClaw/blob/main/CONTRIBUTING.md) to get started. We particularly welcome:
...
Join the conversation on [GitHub Discussions](https://github.com/agentscope-ai/SealClaw/discussions) to suggest or pick up work.
...
SealClaw is released under the [Apache License 2.0](LICENSE).
...
<a href="https://github.com/agentscope-ai/SealClaw/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=agentscope-ai/SealClaw" alt="Contributors" />
</a>
