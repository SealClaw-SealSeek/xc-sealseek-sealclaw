...
[![GitHub 仓库](https://img.shields.io/badge/GitHub-仓库-black.svg?logo=github)](https://github.com/agentscope-ai/SealClaw)
[![PyPI](https://img.shields.io/pypi/v/sealclaw?color=3775A9&label=PyPI&logo=pypi)](https://pypi.org/project/sealclaw/)
[![文档](https://img.shields.io/badge/文档-在线-green.svg?logo=readthedocs&label=Docs)](https://sealclaw.io/)
...
[![最后提交](https://img.shields.io/github/last-commit/agentscope-ai/SealClaw)](https://github.com/agentscope-ai/SealClaw)
...
[![GitHub Star](https://img.shields.io/github/stars/agentscope-ai/SealClaw?style=flat&logo=github&color=yellow&label=Star)](https://github.com/agentscope-ai/SealClaw/stargazers)
[![GitHub Fork](https://img.shields.io/github/forks/agentscope-ai/SealClaw?style=flat&logo=github&color=purple&label=Fork)](https://github.com/agentscope-ai/SealClaw/network)
...
[[文档](https://sealclaw.io/)] [[English](README.md)] [[日本語](README_ja.md)]
...
> **核心能力：**
>
> **全域触达** — 钉钉、飞书、QQ、Discord、iMessage 等频道，一个 SealClaw 按需连接。
>
> **由你掌控** — 记忆与个性化由你掌控，本地或云端均可；定时与协作发往指定频道。
>
> **Skills 扩展** — 内置定时任务，自定义技能目录，SealClaw 自动加载，无绑定。
>
> <details>
> <summary><b>你可以用 SealClaw 做什么</b></summary>
>
> <br>
>
...
[2026-03-12] 我们发布了 v0.0.7！完整更新说明见 [v0.0.7 发布说明](https://agentscope-ai.github.io/SealClaw/release-notes)。
...
[2026-03-09] 我们发布了 v0.0.6！完整更新说明见 [v0.0.6 发布说明](https://agentscope-ai.github.io/SealClaw/release-notes)。

[2026-03-06] 我们发布了 v0.0.5！完整更新说明见 [v0.0.5 发布说明](https://agentscope-ai.github.io/SealClaw/release-notes)。

[2026-03-02] 我们发布了 v0.0.4！完整更新说明见 [v0.0.4 发布说明](https://agentscope-ai.github.io/SealClaw/release-notes)。
...
> - **我想在钉钉 / 飞书 / QQ 里聊**：在控制台中进行 [频道配置](https://sealclaw.io/docs/channels)。
> - **我不想装 Python**：[脚本安装](#脚本安装) 自动管理 Python，或使用 [魔搭一键配置](https://modelscope.cn/studios/fork?target=AgentScope/SealClaw) 云端部署。
...
在浏览器打开 **http://127.0.0.1:8088/** 即可使用控制台（与 SealClaw 对话、配置 Agent）。若要在钉钉、飞书、QQ 等 app 内对话，请参考 [文档](https://sealclaw.io/docs/channels) 接入频道。

![Console](https://img.alicdn.com/imgextra/i4/O1CN01kUEGsC1XolpzIWdsu_!!6000000002971-2-tps-3822-2064.png)

### 脚本安装

无需手动配置 Python，一行命令自动完成安装。脚本会自动下载 uv（Python 包管理器）、创建虚拟环境、安装 SealClaw 及其依赖（含 Node.js 和前端资源）。注意：部分网络环境或企业权限管控下可能无法使用。
...
>    - **找到安装目录**：
>      - 检查 `uv` 是否可用：在 CMD 中输入 `uv --version` ，如果显示版本号，则**只需配置 SealClaw 路径**；如果提示 `'uv' 不是内部或外部命令，也不是可运行的程序或批处理文件。`，则需同时配置两者。
>      - uv路径（任选其一，取决于安装位置，若`uv`不可用则填）：通常在`%USERPROFILE%\.local\bin`、`%USERPROFILE%\AppData\Local\uv`或 Python 安装目录下的 `Scripts` 文件夹
>      - SealClaw路径：通常在 `%USERPROFILE%\.sealclaw\bin` 。
>    - **手动添加到系统的 Path 环境变量**：
...
>   - **配置`uv`环境变量**：将`uv`所在目录和 `%USERPROFILE%\.sealclaw\bin` 添加到系统的 `Path` 变量中。
>   - **重新运行**：打开新终端，再次执行安装脚本以完成 `SealClaw` 安装。
>   - **配置`SealClaw`环境变量**：将 `%USERPROFILE%\.sealclaw\bin` 添加到系统的 `Path` 变量中。

安装完成后，请打开新终端并运行：

```bash
sealclaw init --defaults   # 或：sealclaw init（交互式）
sealclaw app
```
...
# 卸载
sealclaw uninstall          # 保留配置和数据
sealclaw uninstall --purge  # 删除所有内容
```
...
如果你不习惯使用命令行，可以下载并使用 SealClaw 的桌面应用版本，无需手动配置 Python 环境或执行命令。

#### 下载

从 [GitHub Releases](https://github.com/agentscope-ai/SealClaw/releases) 下载桌面应用：
- **Windows**: `SealClaw-Setup-<version>.exe`
- **macOS**: `SealClaw-<version>-macOS.zip` (推荐 Apple Silicon)
...
当你从 Releases 下载 SealClaw macOS 应用时，macOS 可能显示：*"Apple 无法验证 'SealClaw' 不包含恶意软件"*。这是因为应用未经过公证。你仍然可以通过以下方式打开：

- **右键打开（推荐）**
  右键点击（或 Control + 点击）SealClaw 应用 → **"打开"** → 在对话框中再次点击 **"打开"**。这会告诉 Gatekeeper 你信任该应用；之后可以像往常一样双击启动。

- **在系统设置中允许**
  如果仍被阻止，进入 **系统设置 → 隐私与安全性**，向下滚动找到类似 *"已阻止 'SealClaw'，因为无法验证开发者"* 的提示，点击 **"仍要打开"** 或 **"允许"**。
...
详细使用说明、故障排除和常见问题，请参见 [桌面应用指南](https://sealclaw.io/docs/desktop)。

---

### 使用 Docker

镜像在 **Docker Hub**（`agentscope/sealclaw`）。镜像 tag：`latest`（稳定版）；`pre`（PyPI 预发布版）。
...
国内用户也可选用阿里云容器镜像服务 (ACR)：`agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/sealclaw`（tag 相同）。

然后在浏览器打开 **http://127.0.0.1:8088/** 进入控制台。配置、记忆与 Skills 保存在 `sealclaw-data` 卷中；模型配置与 API Key 保存在 `sealclaw-secrets` 卷中。如需传入 API Key（如 `DASHSCOPE_API_KEY`），在 `docker run` 时添加 `-e VAR=value` 或 `--env-file .env`。

> **从容器内连接宿主机上的 Ollama 或其他模型服务**
>
> Docker 容器内的 `localhost` 指向容器自身，而非宿主机。如果 Ollama（或其他模型服务）运行在宿主机上，可通过以下方式让容器内的 SealClaw 访问：
>
...
>   agentscope/sealclaw:latest
> ```
> 然后在 SealClaw **设置 → 模型** 中，将 Base URL 改为 `http://host.docker.internal:<端口>` — 例如 Ollama 填 `http://host.docker.internal:11434`，LM Studio 填 `http://host.docker.internal:1234/v1`。
>
...
**不想本地安装？** 使用 [魔搭创空间](https://modelscope.cn/studios/fork?target=AgentScope/SealClaw) 一键云端配置。请将创空间设为 **非公开**，否则他人可能操纵你的 SealClaw。

### 部署到阿里云 ECS

若希望将 SealClaw 部署在阿里云上，可使用阿里云 ECS 一键部署：打开 [SealClaw 阿里云 ECS 部署链接](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884) 按页面提示操作即可。详细步骤见 [阿里云开发者社区：SealClaw 3 分钟部署你的 AI 助理](https://developer.aliyun.com/article/1713682)。

---

## API Key

若使用**云端大模型**（如 DashScope、ModelScope），在开始对话前必须配置 API Key。未配置有效 Key 前，SealClaw 无法正常工作。详情请参考[官方文档](https://sealclaw.io/docs/models#%E9%85%8D%E7%BD%AE%E4%BA%91%E6%8F%90%E4%BE%9B%E5%95%86)。

**配置方式：**

1. **控制台（推荐）** — 运行 `sealclaw app` 后，打开 **http://127.0.0.1:8088/** → **设置** → **模型**。选择提供商、填写 **API Key**，并启用该提供商与模型。
2. **`sealclaw init`** — 运行 `sealclaw init` 时，会引导你配置 LLM 提供商与 API Key。按提示选择提供商并填写 Key 即可。
3. **环境变量** — 使用 DashScope 时，可在终端或工作目录下的 `.env` 文件中设置 `DASHSCOPE_API_KEY`。

其他工具所需密钥（如网页搜索的 `TAVILY_API_KEY`）可在控制台 **设置 → 环境变量** 中配置，详见 [配置](https://sealclaw.io/docs/config)。
...
SealClaw 可在本机完全本地运行大模型，无需 API Key 或云端服务。详情请见[官方文档](https://sealclaw.io/docs/models#%E6%9C%AC%E5%9C%B0%E6%8F%90%E4%BE%9B%E5%95%86llamacpp--MLX)

| 后端          | 适用场景                          | 安装                                                                 |
| ------------- | --------------------------------- | -------------------------------------------------------------------- |
| **llama.cpp** | 跨平台（macOS / Linux / Windows） | `pip install 'sealclaw[llamacpp]'` 或 `bash install.sh --extras llamacpp` |
| **MLX**       | Apple Silicon（M1/M2/M3/M4）      | `pip install 'sealclaw[mlx]'` 或 `bash install.sh --extras mlx`         |
| **Ollama**    | 跨平台（需要 Ollama 服务运行）    | `pip install 'sealclaw[ollama]'` 或 `bash install.sh --extras ollama`   |
...
| 主题                                                      | 说明                                 |
| --------------------------------------------------------- | ------------------------------------ |
| [项目介绍](https://sealclaw.io/docs/intro)        | SealClaw 是什么、怎么用                 |
| [快速开始](https://sealclaw.io/docs/quickstart)   | 安装与运行（本地或魔搭创空间）       |
| [控制台](https://sealclaw.io/docs/console)        | Web 界面：对话与 Agent 配置          |
| [模型](https://sealclaw.io/docs/models)        | 配置云/本地/自定义提供商          |
| [频道配置](https://sealclaw.io/docs/channels)     | 钉钉、飞书、QQ、Discord、iMessage 等 |
| [Skills](https://sealclaw.io/docs/skills)         | 扩展与自定义能力                     |
| [MCP](https://sealclaw.io/docs/mcp)               | 管理 MCP 客户端                     |
| [记忆](https://sealclaw.io/docs/memory)           | 长期记忆                 |
| [上下文](https://sealclaw.io/docs/context)       | 上下文管理机制                       |
| [魔法命令](https://sealclaw.io/docs/commands)           | 控制对话状态，无需等待AI理解        |
| [心跳](https://sealclaw.io/docs/heartbeat)        | 定时自检与摘要                       |
| [配置与工作目录](https://sealclaw.io/docs/config) | 工作目录与配置文件                   |
| [CLI](https://sealclaw.io/docs/cli)               | 初始化、定时任务、Skills、清理       |
| [FAQ 常见问题](https://sealclaw.io/docs/faq)               | 常见问题与报错排查                   |
...
常见问题、排错指南与已知问题，请访问 **[FAQ 页面](https://sealclaw.io/docs/faq)**。
...
SealClaw 在开放协作中持续演进，欢迎各种形式的参与！请参考上方 [路线图](#路线图)（尤其是标记为 **征集中** 的项）选择你感兴趣的方向，并阅读 [CONTRIBUTING](https://github.com/agentscope-ai/SealClaw/blob/main/CONTRIBUTING_zh.md) 了解如何开始。我们特别欢迎：
...
- **然后**：运行 `sealclaw init --defaults`，再运行 `sealclaw app`。

---

## 为什么叫 SealClaw？

SealClaw 既是「你的搭档小爪子」（co-paw），也寓意 **Co Personal Agent Workstation**（协同个人智能体工作台）。我们希望它不是冰冷的工具，而是一只随时准备帮忙的温暖「小爪子」，是你数字生活中最默契的伙伴。
...
SealClaw 在执行 `sealclaw init` 时会收集**匿名**使用数据，帮助我们了解用户环境并优化产品。数据**每个版本收集一次** — 当你升级 SealClaw 后，会重新收集以便我们了解版本分布。
...
交互式运行 `sealclaw init` 时，会询问你是否同意。使用 `--defaults` 模式则自动同意。提示每个版本仅出现一次，且不影响 SealClaw 的任何功能。

---

## 许可证

SealClaw 采用 [Apache License 2.0](LICENSE) 开源协议。

---

## 贡献者

感谢所有为 SealClaw 做出贡献的朋友们：

<a href="https://github.com/agentscope-ai/SealClaw/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=agentscope-ai/SealClaw" alt="贡献者" />
</a>
