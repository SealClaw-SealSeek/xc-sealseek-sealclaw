...
def create_runtime_router(*, include_auth: bool = True) -> APIRouter:
    """Build the top-level runtime router.

    ``include_auth`` stays ``True`` for the legacy ``/api/*`` surface and
    becomes ``False`` for the new ``/api/v1/*`` runtime surface, where auth
    moves to user-center.
    """
    router = APIRouter()
...
router = create_runtime_router(include_auth=True)
v1_router = create_runtime_router(include_auth=False)
...
__all__ = ["router", "v1_router", "create_agent_scoped_router"]
