...
Public API:
- SealClawAgent: Main agent class
- create_model_and_formatter: Factory for creating models and formatters

Example:
    >>> from sealclaw.agents import SealClawAgent, create_model_and_formatter
    >>> agent = SealClawAgent()
    >>> # Or with custom model
    >>> model, formatter = create_model_and_formatter()
"""

# SealClawAgent is lazy-loaded so that importing agents.skills_manager (e.g.
# from CLI init_cmd/skills_cmd) does not pull react_agent, agentscope, tools.
# pylint: disable=undefined-all-variable
__all__ = ["SealClawAgent", "create_model_and_formatter"]


def __getattr__(name: str):
    """Lazy load heavy imports."""
    if name == "SealClawAgent":
        from .react_agent import SealClawAgent

        return SealClawAgent
