---
name: cron
description: 通过 sealclaw 命令管理定时任务 - 创建、查询、暂停、恢复、删除任务
metadata: { "builtin_skill_version": "1.0", "sealclaw": { "emoji": "⏰" } }
---

# 定时任务管理

使用 `sealclaw cron` 命令管理定时任务。
...
# 查看任务详情
sealclaw cron get <job_id> --agent-id <agent_id>

# 查看任务状态
sealclaw cron state <job_id> --agent-id <agent_id>

# 删除任务
sealclaw cron delete <job_id> --agent-id <agent_id>

# 暂停/恢复任务
sealclaw cron pause <job_id> --agent-id <agent_id>
sealclaw cron resume <job_id> --agent-id <agent_id>

# 立即执行一次
sealclaw cron run <job_id> --agent-id <agent_id>
```
...
- 缺少参数时，询问用户补充后再创建
- 暂停/删除/恢复前，用 `sealclaw cron list` 查找 job_id
- 排查问题时，用 `sealclaw cron state <job_id>` 查看状态
- 给用户的命令要完整、可直接复制执行
- 记得指定 `--agent-id` 参数
