...
def ensure_skills_initialized(workspace_dir: Path) -> None:
    ...
    Skills should be configured via `sealclaw init` or
    `sealclaw skills config`.
    """
    active_skills = get_active_skills_dir(workspace_dir)
    ...
    if not active_skills.exists() or not available:
        logger.warning(
            ...
            "Run 'sealclaw init' or 'sealclaw skills config' "
            "to configure skills.",
        )
    else:
        logger.debug(
...
class SkillService:
    ...
    def import_from_zip(
        ...
        try:
            tmp_dir = Path(
                tempfile.mkdtemp(prefix="sealclaw_skill_upload_"),
            )
