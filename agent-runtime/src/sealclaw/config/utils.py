...
def _normalize_working_dir_bound_paths(data: object) -> object:
    """Normalize legacy ~/.sealclaw-bound paths to current WORKING_DIR.

    This keeps SEALCLAW_WORKING_DIR effective even if user config files contain
    older hard-coded paths like "~/.sealclaw/media" or
    "/Users/x/.sealclaw/workspaces/...".
    Only rewrites known working-dir-bound keys.
    """
    legacy_root_tilde = "~/.sealclaw"
...
def get_available_channels() -> Tuple[str, ...]:
    ...
    sealclaw.channels), filtered by SEALCLAW_ENABLED_CHANNELS or
    SEALCLAW_DISABLED_CHANNELS when set.

    * SEALCLAW_ENABLED_CHANNELS — whitelist (only these channels are active).
    * SEALCLAW_DISABLED_CHANNELS — blacklist (all channels *except* these).
    * If both are set, SEALCLAW_ENABLED_CHANNELS takes precedence.
    * If neither is set, all discovered channels are returned.
    ...
    raw_enabled = os.environ.get("SEALCLAW_ENABLED_CHANNELS", "").strip()
    if raw_enabled:
        enabled = {ch.strip() for ch in raw_enabled.split(",") if ch.strip()}
        return tuple(k for k in all_keys if k in enabled) or all_keys

    raw_disabled = os.environ.get("SEALCLAW_DISABLED_CHANNELS", "").strip()
    if raw_disabled:
...
def is_running_in_container() -> bool:
    ...
    Prefer env SEALCLAW_RUNNING_IN_CONTAINER (1/true/yes) at call time so
