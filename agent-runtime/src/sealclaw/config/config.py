...
class VoiceChannelConfig(BaseChannelConfig):
    ...
    language: str = "en-US"
    welcome_greeting: str = "Hi! This is SealClaw. How can I help you?"
...
def load_agent_config(agent_id: str) -> AgentProfileConfig:
    ...
    # This keeps SEALCLAW_WORKING_DIR effective even if existing agent.json
    # contains older hard-coded paths like "~/.sealclaw/media".
    try:
...
def migrate_legacy_config_to_multi_agent() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default SealClaw agent",
    ...
    # When SEALCLAW_WORKING_DIR is customized, historical data may still exist
    # under "~/.sealclaw".
    old_workspace = Path("~/.sealclaw").expanduser().resolve()
