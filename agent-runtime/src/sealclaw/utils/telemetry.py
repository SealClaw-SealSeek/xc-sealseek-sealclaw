...
def _detect_install_method() -> str:
    """Detect how SealClaw was installed based on environment signals."""
    import os

    if os.environ.get("SEALCLAW_RUNNING_IN_CONTAINER", "").lower() in (
        "1",
        "true",
        "yes",
    ):
        return "docker"
    if os.environ.get("SEALCLAW_DESKTOP_APP", "").lower() in (
        "1",
        "true",
        "yes",
    ):
...
def get_system_info() -> dict[str, Any]:
    ...
    - sealclaw_version: SealClaw version string
    - install_method: How SealClaw was installed (docker/desktop/pip)
    - os: Operating system (Windows/Darwin/Linux)
    - os_version: OS version string
    - python_version: Python version running sealclaw (major.minor)
    - architecture: CPU architecture (x86_64/arm64/etc)
    - has_gpu: GPU availability detection
    """
    from ..__version__ import __version__ as sealclaw_ver

    info = {
        "install_id": str(uuid.uuid4()),
        "sealclaw_version": _safe_get(lambda: sealclaw_ver, "unknown"),
...
def _get_current_version() -> str:
    """Get the current SealClaw version string."""
    try:
        from ..__version__ import __version__ as sealclaw_ver
...
def has_telemetry_been_collected(working_dir: Path) -> bool:
    ...
    Re-triggers collection when SealClaw is upgraded (or downgraded) to a version
    that hasn't been collected before.

    Args:
        working_dir: Path to SealClaw working directory
    ...
    try:
        ...
        # v1.1 compat: single sealclaw_version field
        return marker_data.get("sealclaw_version", "") == current
    except Exception:
        return False
        ...
        if marker_file.exists():
            try:
                ...
                if not collected_versions:
                    old_ver = old_data.get("sealclaw_version", "")
                    if old_ver:
