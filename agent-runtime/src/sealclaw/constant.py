...
# 迁移检测：如果旧目录 ~/.copaw 存在且新目录 ~/.sealclaw 不存在，提示用户迁移
_old_working_dir = Path("~/.copaw").expanduser()
_new_working_dir = Path("~/.sealclaw").expanduser()
if (
    _old_working_dir.exists()
    and not _new_working_dir.exists()
    and "SEALCLAW_WORKING_DIR" not in os.environ
):
    import warnings
    warnings.warn(
        f"检测到旧版配置目录 {_old_working_dir}，请运行以下命令迁移：\n"
        f"  mv {_old_working_dir} {_new_working_dir}\n"
        f"  mv {_old_working_dir}.secret {_new_working_dir}.secret\n"
        "迁移完成后即可正常使用 SealClaw。",
        stacklevel=1,
    )

WORKING_DIR = (
    Path(EnvVarLoader.get_str("SEALCLAW_WORKING_DIR", "~/.sealclaw"))
    .expanduser()
    .resolve()
)
SECRET_DIR = (
...
JOBS_FILE = EnvVarLoader.get_str("SEALCLAW_JOBS_FILE", "jobs.json")

CHATS_FILE = EnvVarLoader.get_str("SEALCLAW_CHATS_FILE", "chats.json")

TOKEN_USAGE_FILE = EnvVarLoader.get_str(
    "SEALCLAW_TOKEN_USAGE_FILE",
    "token_usage.json",
)

CONFIG_FILE = EnvVarLoader.get_str("SEALCLAW_CONFIG_FILE", "config.json")

HEARTBEAT_FILE = EnvVarLoader.get_str("SEALCLAW_HEARTBEAT_FILE", "HEARTBEAT.md")
...
DOCS_ENABLED = EnvVarLoader.get_bool("SEALCLAW_OPENAPI_DOCS", False)

# Memory directory
MEMORY_DIR = WORKING_DIR / "memory"

# Custom channel modules (installed via `sealclaw channels install`); manager
...
MEMORY_COMPACT_KEEP_RECENT = EnvVarLoader.get_int(
    "SEALCLAW_MEMORY_COMPACT_KEEP_RECENT",
    3,
    min_value=0,
)
...
# Example: SEALCLAW_CORS_ORIGINS="http://localhost:5173,http://127.0.0.1:5173"
# When unset, CORS middleware is not applied.
CORS_ORIGINS = EnvVarLoader.get_str("SEALCLAW_CORS_ORIGINS", "").strip()

# LLM API retry configuration
LLM_MAX_RETRIES = EnvVarLoader.get_int(
    "SEALCLAW_LLM_MAX_RETRIES",
    3,
    min_value=0,
)

LLM_BACKOFF_BASE = EnvVarLoader.get_float(
    "SEALCLAW_LLM_BACKOFF_BASE",
    1.0,
    min_value=0.1,
)

LLM_BACKOFF_CAP = EnvVarLoader.get_float(
    "SEALCLAW_LLM_BACKOFF_CAP",
    10.0,
    min_value=0.5,
)
...
try:
    TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS = max(
        float(
            os.environ.get("SEALCLAW_TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS", "600"),
        ),
        1.0,
    )
except (TypeError, ValueError):
