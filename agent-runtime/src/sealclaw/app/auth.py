...
variable ``SEALCLAW_AUTH_ENABLED`` is set to a truthy value (``true``,
...
def is_auth_enabled() -> bool:
    ...
    Returns ``True`` when ``SEALCLAW_AUTH_ENABLED`` is set to a truthy
    ...
    """
    env_flag = os.environ.get("SEALCLAW_AUTH_ENABLED", "").strip().lower()
...
def auto_register_from_env() -> None:
    ...
    Called once during application startup.  If ``SEALCLAW_AUTH_ENABLED``
    is truthy and both ``SEALCLAW_AUTH_USERNAME`` and ``SEALCLAW_AUTH_PASSWORD``
    ...
    username = os.environ.get("SEALCLAW_AUTH_USERNAME", "").strip()
    password = os.environ.get("SEALCLAW_AUTH_PASSWORD", "").strip()
