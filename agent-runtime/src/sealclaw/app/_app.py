...
from ..utils.logging import setup_logger, add_sealclaw_file_handler
...
from .request_identity import RequestIdentityMiddleware
...
# Console static dir: env, or sealclaw package data (console), or cwd.
_CONSOLE_STATIC_ENV = "SEALCLAW_CONSOLE_STATIC_DIR"


def _resolve_console_static_dir() -> str:
    if os.environ.get(_CONSOLE_STATIC_ENV):
        return os.environ[_CONSOLE_STATIC_ENV]
    # Shipped dist lives in sealclaw package as static data (not a Python pkg).
    pkg_dir = Path(__file__).resolve().parent.parent
    candidate = pkg_dir / "console"
    if candidate.is_dir() and (candidate / "index.html").exists():
        return str(candidate)
    # the following code can be removed after next release,
    # because the console will be output to sealclaw's
    # `src/sealclaw/console/` directory directly by vite.
    cwd = Path(os.getcwd())
...
def read_root():
    ...
    return {
        "message": (
            "SealClaw Web Console is not available. "
            "If you installed SealClaw from source code, please run "
            "`npm ci && npm run build` in SealClaw's `console/` "
            "directory, and restart SealClaw to enable the "
            "web console."
        ),
    }
...
@app.get("/api/version")
def get_version():
    """Return the current SealClaw version."""
    return {"version": __version__}


@app.get("/api/v1/version")
def get_v1_version():
    """Return the current SealClaw version for the v1 API surface."""
    return {"version": __version__}
...
app.include_router(agent_scoped_router, prefix="/api/v1")
...
if os.path.isdir(_CONSOLE_STATIC_DIR):
    ...
    @app.get("/sealclaw-symbol.svg")
    def _console_icon():
        f = _console_path / "sealclaw-symbol.svg"
        if f.is_file():
            return FileResponse(f, media_type="image/svg+xml")
        raise HTTPException(status_code=404, detail="Not Found")

    @app.get("/sealclaw-dark.png")
    def _console_dark_icon():
        f = _console_path / "sealclaw-dark.png"
        if f.is_file():
            return FileResponse(f, media_type="image/png")
        raise HTTPException(status_code=404, detail="Not Found")
