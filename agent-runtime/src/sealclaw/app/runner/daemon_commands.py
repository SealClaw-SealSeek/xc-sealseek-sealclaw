...
Shared by in-chat /daemon <sub> and CLI `sealclaw daemon <sub>`.
Logs: tail WORKING_DIR / "sealclaw.log". Restart: in-process reload of channels,
...
async def run_daemon_restart(context: DaemonContext) -> str:
    ...
    return (
        ...
        "Run the app (e.g. `sealclaw app`) and use /daemon restart in chat, "
...
def run_daemon_version(context: DaemonContext) -> str:
    ...
    return (
        ...
        f"- Log file: {context.working_dir / 'sealclaw.log'}"
    )


def run_daemon_logs(context: DaemonContext, lines: int = 100) -> str:
    """Tail last N lines from WORKING_DIR / sealclaw.log."""
    log_path = context.working_dir / "sealclaw.log"
