...
async def post_console_chat(
    ...
    if identity is not None and getattr(identity, "user_id", ""):
        if not native_payload["sender_id"] or native_payload["sender_id"] == "default":
            native_payload["sender_id"] = identity.user_id
        native_payload["meta"]["user_id"] = identity.user_id
    if identity is not None and getattr(identity, "workspace_id", ""):
        native_payload["meta"]["workspace_id"] = identity.workspace_id
