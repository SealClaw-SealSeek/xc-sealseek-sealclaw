...
def _matches_sealclaw_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a SealClaw CLI invocation."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for pattern in (
            f" -m sealclaw {subcommand}",
            f" sealclaw {subcommand}",
            f"__main__.py {subcommand}",
            f'sealclaw.exe" {subcommand}',
            f"sealclaw.exe {subcommand}",
        )
    )


def _is_sealclaw_service_command(command: str) -> bool:
    """Return whether the command line looks like a local SealClaw app."""
    return _matches_sealclaw_cli_command(command, "app")


def _is_sealclaw_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a SealClaw CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "sealclaw" or _matches_sealclaw_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local SealClaw service."""
    if not host:
        return []
