# -*- coding: utf-8 -*-
"""sealclaw uninstall — remove the SealClaw environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove SealClaw PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# SealClaw" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# SealClaw\nexport PATH=\"\$HOME/\.sealclaw/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove SealClaw environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL SealClaw data in {wd}")
    else:
        click.echo(
            "This will remove the SealClaw Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("SealClaw uninstalled. Please restart your terminal.")
