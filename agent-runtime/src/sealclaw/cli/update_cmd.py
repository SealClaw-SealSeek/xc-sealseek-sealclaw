...
from .process_utils import (
    ...
    _is_sealclaw_service_command,
    _process_table,
)

_PYPI_JSON_URL = "https://pypi.org/pypi/sealclaw/json"
...
class InstallInfo:
    """Information about the current SealClaw installation."""
...
class RunningServiceInfo:
    """Detected SealClaw service endpoint state."""
...
def _fetch_latest_version() -> str:
    """Fetch the latest published SealClaw version from PyPI."""
    try:
        resp = httpx.get(
            _PYPI_JSON_URL,
            timeout=10.0,
        ...
        raise click.ClickException(
            f"Failed to fetch the latest SealClaw version from PyPI: {exc}",
        ) from exc
    except json.JSONDecodeError as exc:
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            f"latest SealClaw version: {exc}",
        ) from exc
    version = str(data.get("info", {}).get("version", "")).strip()
    if not version:
        raise click.ClickException(
            "Unable to determine the latest SealClaw version.",
        )
    return version
...
def _detect_installation() -> InstallInfo:
    ...
    dist = metadata.distribution("sealclaw")
...
def _probe_service(base_url: str) -> RunningServiceInfo:
    """Probe a possible running SealClaw HTTP service."""
    try:
        resp = httpx.get(
...
def _process_candidate_ports() -> list[int]:
    """Infer candidate local SealClaw service ports from running processes."""
    ports: list[int] = []
    for _pid, command in _process_table():
        if not _is_sealclaw_service_command(command):
            continue
...
def _detect_running_service(
    ...
    """Detect whether a SealClaw HTTP service is currently running."""
    candidates: list[str] = []
    seen: set[str] = set()
    preferred_hosts: list[str] = []
...
def _running_service_display(running: RunningServiceInfo) -> str:
    ...
    if not running.base_url:
        return "a running SealClaw service"
    version_suffix = f" (version {running.version})" if running.version else ""
    return f"SealClaw service at {running.base_url}{version_suffix}"


def _confirm_force_shutdown(running: RunningServiceInfo) -> bool:
    """Ask whether `sealclaw shutdown` should be used before updating."""
    click.echo("")
    ...
    click.secho(
        "Running `sealclaw shutdown` will forcibly terminate the current "
        "SealClaw backend/frontend processes.",
        fg="red",
        bold=True,
    )
    click.secho(
    ...
    return click.confirm(
        "Run `sealclaw shutdown` now and continue with the update?",
        default=False,
    )
...
def _run_shutdown_for_update(
    ...
    """Run `sealclaw shutdown` in the current environment before updating."""
    command = [info.python_executable, "-m", "sealclaw"]
    ...
    click.echo("")
    click.echo("Running `sealclaw shutdown` before updating...")
    ...
    try:
        ...
        raise click.ClickException(
            "Failed to run `sealclaw shutdown`: " f"{exc}",
        ) from exc
    ...
    if result.returncode != 0:
        raise click.ClickException(
            "`sealclaw shutdown` failed. Please stop the running SealClaw "
            "service manually before running `sealclaw update`.",
        )
...
def _build_upgrade_command(
    ...
    package_spec = f"sealclaw=={latest_version}"
...
def _spawn_update_worker(
    ...
    *,
    ...
    worker_code = (
        "from sealclaw.cli.update_cmd import run_update_worker; "
        "import sys; "
...
def _run_update_worker_foreground(plan_path: Path) -> int:
    ...
    try:
        ...
        click.echo("")
        click.echo("[sealclaw] Update interrupted. Stopping installer...")
...
def run_update_worker(plan_path: str | Path) -> int:
    ...
    click.echo("")
    click.echo(
        "[sealclaw] Updating SealClaw "
        f"{plan['current_version']} -> {plan['latest_version']}...",
    )
    click.echo(f"[sealclaw] Using installer: {plan['installer_label']}")
    ...
    try:
        ...
        click.echo(f"[sealclaw] Update failed: {exc}")
        return_code = 1
    finally:
        try:
    ...
    if return_code == 0:
        click.echo("[sealclaw] Update completed successfully.")
        click.echo(
            "[sealclaw] Please restart any running SealClaw service "
            "to use the new version.",
        )
    else:
        click.echo(f"[sealclaw] Update failed with exit code {return_code}.")
        click.echo(
            "[sealclaw] Please fix the error above and run "
            "`sealclaw update` again.",
        )
...
def update_cmd(ctx: click.Context, yes: bool) -> None:
    """Upgrade SealClaw in the current Python environment."""
    ...
    if version_check is False:
        click.echo("SealClaw is already up to date.")
        return
    ...
    if running.is_running:
        if yes:
            raise click.ClickException(
                ...
                "Please stop it before running `sealclaw update`, or rerun "
                "without `--yes` to confirm a forced `sealclaw shutdown`.",
            )
        ...
        if running.is_running:
            raise click.ClickException(
                "Detected "
                f"{_running_service_display(running)} after `sealclaw shutdown`. "
                "Please stop it manually before running `sealclaw update`.",
            )

    if not yes and not click.confirm(
        f"Update SealClaw to {latest_version} in the current environment?",
        default=True,
    ):
        click.echo("Cancelled.")
        return
