# -*- coding: utf-8 -*-
"""
Skill security scanner for SealClaw.
...
def _get_scan_mode(cfg: Any = None) -> str:
    ...
    Priority: env ``SEALCLAW_SKILL_SCAN_MODE`` > config > default ``warn``.
    """
    env = os.environ.get("SEALCLAW_SKILL_SCAN_MODE")
    if env is not None:
        val = env.lower().strip()
...
def _get_blocked_history_path() -> Path:
    try:
        ...
        return Path.home() / ".sealclaw" / _BLOCKED_HISTORY_FILE
