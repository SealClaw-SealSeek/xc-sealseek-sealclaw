# SealClaw Desktop Application Guide
...
This guide explains how to install and use the SealClaw Desktop application on Windows and macOS.
...
1. **Download the installer**
   Download `SealClaw-Setup-<version>.exe` from the [Release page][releases]

2. **Run the installer**
   Double-click the `.exe` file and follow the installation wizard
   - Default installation location: `C:\Users\<your-username>\AppData\Local\SealClaw`
...
#### **SealClaw Desktop** (Recommended for daily use)
...
- **How to Launch**: Double-click the "SealClaw Desktop" icon on desktop or Start Menu
- **Technical Note**: Uses VBScript launcher, runs Python process in background

#### **SealClaw Desktop (Debug)** (Debug Mode)
...
- **How to Launch**: Double-click "SealClaw Desktop (Debug)" icon in Start Menu
- **Log Contents**:
...
A: Use "SealClaw Desktop (Debug)" mode to view terminal output for error messages

**Q: How to uninstall?**
A: Go to Windows Settings → Apps → Installed apps → Find "SealClaw Desktop" → Uninstall
...
1. **Download the archive**
   Download `SealClaw-<version>-macOS.zip` from the [Release page][releases]

2. **Extract**
   Double-click the `.zip` file to extract and get `SealClaw.app`

3. **Move to Applications folder (Optional)**
   Drag `SealClaw.app` to the `/Applications` folder

### First Launch: Bypassing System Security Restrictions

#### Why manual trust is needed?

SealClaw is **not Apple Developer-signed or notarized**, so macOS Gatekeeper will block it by default.
...
1. **Right-click** (or Control + click) on `SealClaw.app`
...
2. Scroll down to find a message like:
   _"'SealClaw' was blocked from use because it cannot verify the developer"_
...
- Double-click `SealClaw.app` to launch
- The app runs in the background and opens a browser window
- Logs are written to: `~/.sealclaw/desktop.log`
...
cd /Applications  # or wherever your SealClaw.app is located

# Set environment variables and launch
APP_ENV="$(pwd)/SealClaw.app/Contents/Resources/env"
PYTHONPATH= PYTHONHOME="$APP_ENV" "$APP_ENV/bin/python" -m sealclaw desktop
```
...
1. Check the `~/.sealclaw/desktop.log` file for errors
...
**Q: How to uninstall?**
A: Drag `SealClaw.app` to the Trash, then delete the `~/.sealclaw` configuration folder
...
- **GitHub Issues**: [Submit an issue](https://github.com/agentscope-ai/SealClaw/issues)
- **Packaging documentation**: `scripts/pack/README.md` - Technical details and local build guide
- **Log locations**:
  - Windows: View in Debug mode terminal, or `%USERPROFILE%\.sealclaw\` directory
  - macOS: `~/.sealclaw/desktop.log`
