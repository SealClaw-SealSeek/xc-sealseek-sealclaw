...
This page describes what SealClaw is, what it can do, and how to get started by
following the docs.
...
SealClaw is a **personal assistant** that runs in your own environment.
...
SealClaw is built by the [AgentScope team](https://github.com/agentscope-ai) on
...
## How do you use SealClaw?

You use SealClaw in two main ways:

1. **Chat in your messaging apps**
   Send messages in DingTalk, Feishu, QQ, Discord, or iMessage (Mac only); SealClaw replies
   in the same app and can look things up, manage todos, answer questions —
   whatever the enabled Skills support. One SealClaw instance can be connected to
   several apps; it replies in the channel where you last talked.

2. **Run on a schedule**
   Without sending a message each time, SealClaw can run at set times:
   - Send a fixed message to a channel (e.g. “Good morning” to DingTalk at 9am);
   - Ask SealClaw a question and send the answer to a channel (e.g. every 2 hours
     ask “What are my todos?” and post the reply to DingTalk);
   - Run a “check-in” or digest: ask SealClaw a block of questions you wrote and
     send the answer to the channel you last used.

After you install, connect at least one channel, and start the server, you can
chat with SealClaw in DingTalk, Feishu, QQ, etc. and use scheduled messages and check-ins;
...
- **Channel** — Where you talk to SealClaw (DingTalk, Feishu, QQ, Discord, iMessage, etc.).
  Configure each in [Channels](./channels).
- **Heartbeat** — On a fixed interval, ask SealClaw a block of text you wrote and
...
- **Cron jobs** — Scheduled tasks (send X at 9am, ask Y every 2h, etc.), managed
  via [CLI](./cli) or API.
- **Agent/Workspace** — Starting from **v0.1.0**, SealClaw supports multi-agent workspace,
