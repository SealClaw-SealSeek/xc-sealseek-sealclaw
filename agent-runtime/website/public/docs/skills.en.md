...
`~/.sealclaw/active_skills/`): any subdirectory containing a `SKILL.md` is loaded as a
...
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| **cron**                     | Scheduled jobs. Create, list, pause, resume, or delete jobs via `sealclaw cron` or Console **Control → Cron Jobs**; run on a schedule and send results to a channel.           | Built-in                                                       |
...
`~/.sealclaw/active_skills/cron/`. It provides “run on a schedule and send results to a
channel.” You manage jobs with the [CLI](./cli) (`sealclaw cron`) or in the
Console under **Control → Cron Jobs**; no need to edit skill files.

Common operations:

- Create a job: `sealclaw cron create --type agent --name "xxx" --cron "0 9 * * *" ...`
- List jobs: `sealclaw cron list`
- Check state: `sealclaw cron state <job_id>`
...
1. Create a directory under `~/.sealclaw/customized_skills/`, e.g. `my_skill`.
...
On startup the app merges built-in skills with custom skills from `~/.sealclaw/customized_skills/` into `~/.sealclaw/active_skills/`; custom skills take priority when names collide. Your custom directories are never overwritten; built-in skills are only copied to `active_skills` when missing.
