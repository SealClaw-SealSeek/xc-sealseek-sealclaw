...
Then open a new terminal (the installer adds SealClaw to your PATH automatically).
...
To upgrade, simply re-run the install command. To uninstall, run `sealclaw uninstall`.

### Step 2: Init

Generate `config.json` and `HEARTBEAT.md` in the working directory (default
`~/.sealclaw`). Two options:
...
To overwrite existing config, use `sealclaw init --force` (you will be prompted).
...
configured a channel, SealClaw will reply there; otherwise you can add one after
...
`source .venv/bin/activate` on Linux/macOS or `.venv\Scripts\Activate.ps1` on Windows). This installs the `sealclaw` command.
...
1. **Download the installer**
   Go to [GitHub Releases](https://github.com/agentscope-ai/SealClaw/releases) to download the version for your system:

   - Windows: `SealClaw-Setup-<version>.exe`
   - macOS: `SealClaw-<version>-macOS.zip`

2. **Install and Launch**

   - **Windows**: Double-click the `.exe` file to install following the wizard, then double-click the desktop shortcut to launch
   - **macOS**: Extract the `.zip` to get `SealClaw.app`, first time requires right-click and select "Open" to bypass system security restrictions
...
If you prefer not to install Python locally, you can deploy SealClaw to ModelScope Studio's cloud:

1. First, sign up and log in at [ModelScope](https://modelscope.cn/register?back=%2Fhome);
2. Open the [SealClaw Studio](https://modelscope.cn/studios/fork?target=AgentScope/SealClaw) and complete the one-click setup.
...
Images are on **Docker Hub** (`agentscope/sealclaw`). Image tags: `latest` (stable); `pre` (PyPI pre-release). Also available on Alibaba Cloud ACR for users in China: `agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/sealclaw` (same tags).
...
Then open **http://127.0.0.1:8088/** in your browser for the Console. Config, memory, and skills are stored in the `sealclaw-data` volume; model provider settings and API keys are in the `sealclaw-secrets` volume. To pass API keys, add `-e DASHSCOPE_API_KEY=xxx` or `--env-file .env` to `docker run`.

---

## Option F: Deploy on Alibaba Cloud ECS

To run SealClaw on Alibaba Cloud, use the ECS one-click deployment:

1. Open the [SealClaw on Alibaba Cloud (ECS) deployment link](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884) and fill in the parameters as prompted;
