...
SealClaw divides the context into three zones:
...
- [MemoryCompactionHook](https://github.com/agentscope-ai/SealClaw/blob/main/src/sealclaw/agents/hooks/memory_compaction.py)
...
When the context approaches its limit, SealClaw automatically triggers compaction, condensing old conversations into a structured summary.
...
Configuration is located in `~/.sealclaw/config.json` under `agents.running`:
