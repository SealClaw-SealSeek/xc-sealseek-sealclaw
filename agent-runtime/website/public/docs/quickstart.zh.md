...
- **方式六 — 阿里云 ECS**：在阿里云上一键部署 SealClaw，无需本地安装。
...
然后打开新终端（安装脚本会自动将 SealClaw 加入 PATH）。
...
升级只需重新运行安装命令。卸载请运行 `sealclaw uninstall`。

### 步骤二：初始化

在工作目录（默认 `~/.sealclaw`）下生成 `config.json` 与 `HEARTBEAT.md`。两种方式：
...
若已有配置想覆盖，可使用 `sealclaw init --force`（会提示确认）。
...
服务默认监听 `127.0.0.1:8088`。若已配置频道，SealClaw 会在对应 app 内回复；若尚未配置，也可先完成本节再前往频道配置。
...
`source .venv/bin/activate`，Windows 下 `.venv\Scripts\Activate.ps1`）。安装后会提供 `sealclaw` 命令。
...
1. **下载安装包**
   前往 [GitHub Releases](https://github.com/agentscope-ai/SealClaw/releases) 下载对应系统的版本：
...
2. **安装并启动**
   ...
   - **macOS**: 解压 `.zip` 得到 `SealClaw.app`，首次需右键选择"打开"以绕过系统安全限制
...
若不想在本地安装 Python，可通过魔搭创空间将 SealClaw 部署到云端运行：

1. 先前往 [魔搭](https://modelscope.cn/register?back=%2Fhome) 注册并登录；
2. 打开 [SealClaw 创空间](https://modelscope.cn/studios/fork?target=AgentScope/SealClaw)，一键配置即可使用。

**重要**：使用创空间请将空间设为 **非公开**，否则你的 SealClaw 可能被他人操纵。

---

## 方式五：Docker

镜像在 **Docker Hub**（`agentscope/sealclaw`）。镜像 tag：`latest`（稳定版）；`pre`（PyPI 预发布版）。国内用户也可选用阿里云 ACR：`agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/sealclaw`（tag 相同）。
...
然后在浏览器打开 **http://127.0.0.1:8088/** 进入控制台。配置、记忆与 Skills 保存在 `sealclaw-data` 卷中；模型配置与 API Key 保存在 `sealclaw-secrets` 卷中。传入 API Key 可在 `docker run` 时加 `-e DASHSCOPE_API_KEY=xxx` 或 `--env-file .env`。

---

## 方式六：部署到阿里云 ECS

若希望将 SealClaw 部署在阿里云上，可使用阿里云 ECS 一键部署：

1. 打开 [SealClaw 阿里云 ECS 部署链接](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884)，按页面提示填写部署参数；
2. 参数配置完成后确认费用并创建实例，部署完成后即可获取访问地址并使用服务。

详细步骤与说明请参考 [阿里云开发者社区：SealClaw 3 分钟部署你的 AI 助理](https://developer.aliyun.com/article/1713682)。
